...
import { PageHeader, ImageUpload, GalleryUpload } from '../../components/shared';
  ...
  useEffect(() => {
    if (project) {
      reset({
        ...
        technologies: (project.technologies as any[]).map((t) => (typeof t === 'string' ? t : t._id)),
        images: {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Tabs defaultValue="basic" className="space-y-6">
          <TabsList className="bg-slate-800 border border-slate-700">
            <TabsTrigger value="basic" className="data-[state=active]:bg-slate-700">المعلومات الأساسية</TabsTrigger>
            <TabsTrigger value="details" className="data-[state=active]:bg-slate-700">التفاصيل</TabsTrigger>
            <TabsTrigger value="media" className="data-[state=active]:bg-slate-700">الوسائط</TabsTrigger>
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label className="text-slate-200">الرابط *</Label>
                    <Input
                      {...register('slug')}
                      className="bg-slate-700/50 border-slate-600 text-white"
                ...
                <div className="space-y-2">
                  <Label className="text-slate-200">الملخص *</Label>
                  <Textarea
                    {...register('summary')}
                ...
                <div className="space-y-2">
                  <Label className="text-slate-200">رابط المشروع</Label>
                  <Input
                    {...register('projectUrl')}
                    className="bg-slate-700/50 border-slate-600 text-white"
                ...
                <div className="space-y-2">
                  <Label className="text-slate-200">التقنيات المستخدمة</Label>
                  <Controller
                    name="technologies"
                    control={control}
                    render={({ field }) => (
                      <div className="flex flex-wrap gap-2">
                        {technologies?.map((tech) => (
                          <Button
                            key={tech._id}
                            type="button"
                            variant={field.value.includes(tech._id) ? 'default' : 'outline'}
                            size="sm"
                            className={field.value.includes(tech._id) 
                ...
                <div className="flex items-center gap-6 pt-4">
                  <Controller
                    ...
                    render={({ field }) => (
                      <div className="flex items-center gap-2">
                        ...
                        <Label className="text-slate-200">منشور</Label>
                      </div>
                    )}
                  />
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <div className="flex items-center gap-2">
                        <Switch checked={field.value} onCheckedChange={field.onChange} />
                        <Label className="text-slate-200">مميز</Label>
                      </div>
                    )}
                  />
                </div>
          ...
          <TabsContent value="details">
            <Card className="bg-slate-800/50 border-slate-700">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-slate-200">التحدي</Label>
                  <Textarea
                    {...register('challenge')}
                ...
                <div className="space-y-2">
                  <Label className="text-slate-200">الحل</Label>
                  <Textarea
                    {...register('solution')}
                    className="bg-slate-700/50 border-slate-600 text-white min-h-32"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <Label className="text-slate-200">النتائج</Label>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
          ...
          <TabsContent value="media">
            <Card className="bg-slate-800/50 border-slate-700">
              <CardHeader>
                <CardTitle className="text-white">الصور والوسائط</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  <Label className="text-slate-200">صورة الغلاف</Label>
                  <Controller
                    name="images.cover"
                    control={control}
                    render={({ field }) => (
                ...
                <div className="space-y-2">
                  <Label className="text-slate-200">معرض الصور</Label>
                  <Controller
                    name="images.gallery"
                    control={control}
                    render={({ field }) => (
                    ...
                    {watch('seo.keywords')?.map((keyword, index) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() => {
                            const current = watch('seo.keywords') || [];
                            setValue('seo.keywords', current.filter((_, i) => i !== index));
                          }}
