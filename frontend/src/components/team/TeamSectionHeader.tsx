  ...
  return (
    ...
    >
      ...
      <h2 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold leading-tight text-white mb-4 sm:mb-6">
        {tr("العقول التي تبني خلف")}<br className="hidden sm:inline" />{" "}
        {tr("كل")}<span className="text-cyan-300">{tr("تجربة رقمية ناجحة")}</span>
      </h2>

      <p className="text-sm sm:text-base md:text-lg text-gray-300 max-w-3xl mx-auto leading-7 sm:leading-8">
        {tr("فريق وكالة سمارت يجمع بين التصميم، البرمجة، التشغيل، والتفكير المنتجاني لبناء حلول رقمية لا تبدو جميلة فقط، بل تعمل بكفاءة وتكبر مع عملك.")}</p>
    </motion.div>
  );
}
