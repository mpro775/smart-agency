...
export default function ArticleCTA({ blog }: { blog: Blog }) {
  const title = blog.ctaTitle || tr("حوّل فكرتك إلى منتج رقمي واضح");
  const description =
    blog.ctaDescription || tr("فريق Smart Agency يساعدك على التخطيط، التصميم، التطوير، والإطلاق بمنهج عملي ونتائج مضمونة.");
  const buttonText = blog.ctaButtonText || tr("ابدأ مشروعك");
